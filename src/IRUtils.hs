...
subst idx sub_e (Match e s cases) =
  let e' = subst idx sub_e e
      cases' =
        (\c -> case_expr %~ (subst (idx + (c ^. case_args . to length)) sub_e)) <$>
        cases
   in Match e' s cases
...
subst idx sub_e (Lambda s e1 e2) =
  Lambda s (subst idx sub_e e1) (subst (idx + 1) sub_e e2)
subst _ _ Type = Type
...
getConstrAndArgs (Call a b) =
  let (constr, args) = getConstrAndArgs a
   in (constr, args ++ [b])
...
callByValue p (Match e ind_name cases) =
  let e' = callByValue p e
   ...
   in callByValue p $ foldl Call lambda_cas args
