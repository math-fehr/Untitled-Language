...
getExprFromIdent str (PIRContext local def ind constrs)
  ...
  | str `M.member` constrs =
    let (constr, idx) = constrs M.! str
     in Just $ Constructor constr idx
  | otherwise = Nothing
...
addInd (PInductive name args ind_constrs) ctx = do
  ...
  let ctx_with_args =
        Prelude.foldl
          (\ctx' (arg_name, _) -> addLocalVar arg_name ctx')
          ctx_with_ind
          args
  ctx' <-
    foldM
      (\ctx' (idx, PInductiveConstructor constr _) ->
...
argsToIr ::
     [(String, Parser.Expr)]
  -> PIRContext
  -> Either Error ([(String, IR.Expr)], PIRContext)
argsToIr args ctx =
  foldM
    (\(l, ctx') (arg_name, arg_typ) -> do
...
constrToIr ::
     PInductiveConstructor -> PIRContext -> Either Error InductiveConstructor
...
indToIr (PInductive name args constr) ctx = do
  (args', ctx') <- argsToIr args ctx
  constr' <- foldM (\l constr' -> (: l) <$> constrToIr constr' ctx') [] constr
