...
argsToIr :: [(String, Parser.Expr, Bool)] -> PIRContext -> Either Error ([(DebugInfo String, IR.Arg)], PIRContext)
argsToIr args ctx =
    foldM
      (\(l, ctx') (arg_name, arg_typ, b) ->
          do expr <- exprToIr arg_typ ctx'
             return (l ++ [(DI arg_name, makeArg b expr)], addLocalVar arg_name ctx'))
      ([], ctx) args
  where makeArg :: Bool -> IR.Expr -> Arg
        makeArg True  = LinearArg
        makeArg False = UnrestrictedArg
  
argsFromType :: Parser.Expr -> [(String, Parser.Expr, Bool)]
argsFromType (Parser.Call (Parser.Call (Parser.Var "->") (Parser.TypedVar name typ)) expr) = (name, typ, False) : argsFromType expr 
argsFromType (Parser.Call (Parser.Call (Parser.Var "-o") (Parser.TypedVar name typ)) expr) = (name, typ, True) : argsFromType expr 
argsFromType _ = []
...
parsedProgramToIr p =
  do ctx <- getProgramCtx p
     foldM (\p' def ->
              flip insertDefinition p' <$> defToIr def ctx)
       (IR.Program M.empty) p
