...
isBooleanDef (Definition _ args typ _) =
  typ == Const BoolType && all ((== Const BoolType) . snd) args
...
generateAllBoolCombinations n =
  let combs = generateAllBoolCombinations (n - 1)
   in fmap (True :) combs ++ fmap (False :) combs
...
runBooleanDef p def =
  if not (isBooleanDef def)
    then return ()
    else let all_booleans = generateAllBoolCombinations (length $ def_args def)
             exprs = fmap (callBoolList (getExprFromDef def)) all_booleans
             results = zip all_booleans $ fmap (callByValue p) exprs
             print_results =
               foldM
                 (\_ (inputs, output) ->
                    putStrLn $ show inputs ++ " " ++ show (toBoolean output))
                 ()
                 results
          in print_results
...
runBooleanProgram p =
  foldM
    (\_ def ->
       putStrLn ("Function " ++ show (def_name def)) >> runBooleanDef p def >>
       putStrLn "")
    ()
    (_prog_defs p)
