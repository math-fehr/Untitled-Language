...
newtype TypingContext =
  TypingContext [Expr]

-- A context with no local variables
...
checkBigArrowWellTyped ::
     TypingContext -> Program -> [Expr] -> Either Error Expr
checkBigArrowWellTyped ctx p es = do
  checkExprWellTypedHasType ctx p (getBigArrow es) Type
...
-- some given type
checkExprWellTypedHasType ::
     TypingContext -> Program -> Expr -> Expr -> Either Error ()
checkExprWellTypedHasType ctx p e expected_typ = do
  typ <- checkExprWellTyped ctx p e
  if expected_typ == typ
    then return ()
    else Left $ ShouldBeType e typ expected_typ
...
checkCaseWellTyped ::
     TypingContext -> Program -> String -> MatchCase -> Either Error Expr
checkCaseWellTyped ctx p ind_name (MatchCase args expr) =
  let ind = getInductive ind_name p
      ctx' = foldl (flip addVarType) ctx (snd <$> ind_args ind)
   in checkExprWellTyped ctx' p expr

-- Check that the term in a branch of a pattern matching is well typed,
-- and that its type is equal to some given type
checkCaseWellTypedHasType ::
     TypingContext -> Program -> String -> MatchCase -> Expr -> Either Error ()
checkCaseWellTypedHasType ctx p ind_name e expected_typ = do
  typ <- checkCaseWellTyped ctx p ind_name e
  if expected_typ == typ
    then return ()
    else Left $ ShouldBeType (e ^. case_expr) typ expected_typ
...
checkExprWellTyped ctx p (Constructor s idx) =
  checkBigArrowWellTyped ctx p $
  (snd <$> (constr_args $ ind_constr (getInductive s p) !! idx)) ++
  [InductiveType s]
checkExprWellTyped _ _ (Const c) = return $ getConstType c
checkExprWellTyped ctx p (Assign _ sub_e e) = do
  sub_e_type <- checkExprWellTyped ctx p sub_e
  checkExprWellTyped (addVarType sub_e_type ctx) p e
checkExprWellTyped ctx p (IfThenElse cond e1 e2) = do
  checkExprWellTypedHasType ctx p cond (Const BoolType)
  e1_type <- checkExprWellTyped ctx p e1
  checkExprWellTypedHasType ctx p e2 e1_type
  return e1_type
checkExprWellTyped _ _ (Match _ _ []) =
  error "Can't check type of a pattern matching with no cases"
checkExprWellTyped ctx p (Match expr ind_name (hd_cases:cases)) = do
  checkExprWellTypedHasType ctx p expr (InductiveType ind_name)
  typ <- checkCaseWellTyped ctx p ind_name hd_cases
  traverse_ (\x -> checkCaseWellTypedHasType ctx p ind_name x typ) cases
  return typ
checkExprWellTyped ctx p (Call fun arg) = do
  fun_typ <- checkExprWellTyped ctx p fun
  case fun_typ of
    Arrow arg_type res_type -> do
      checkExprWellTypedHasType ctx p arg arg_type
      return res_type
    typ -> Left $ NotAFunction fun typ
checkExprWellTyped ctx p (Arrow e1 e2) = do
  checkExprWellTypedHasType ctx p e1 Type
  checkExprWellTypedHasType ctx p e2 Type
  return Type
checkExprWellTyped ctx p (Lambda _ e1 e2) = do
  checkExprWellTypedHasType ctx p e1 Type
...
checkDefWellTyped' ::
     TypingContext -> Program -> Definition -> Either Error Expr
checkDefWellTyped' ctx p (Definition _ [] typ body) = do
  checkExprWellTypedHasType ctx p typ Type
  checkExprWellTypedHasType ctx p body typ
  return typ
checkDefWellTyped' ctx p (Definition name ((_, arg):args) typ body) = do
  checkExprWellTypedHasType ctx p arg Type
  def_typ <-
    checkDefWellTyped' (addVarType arg ctx) p (Definition name args typ body)
  return $ Arrow arg def_typ

-- Check that an inductive constructor declaration is well typed
checkIndConstrWellTyped ::
     TypingContext
  -> Program
  -> String
...
checkProgramWellTyped p@(Program defs inds) =
  traverse_ (checkDefWellTyped p) defs >> traverse_ (checkIndWellTyped p) inds
